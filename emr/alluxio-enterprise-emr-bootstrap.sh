...
wait_for_hadoop() {
  ...
  hdfs_pid="XX"
  echo "Waiting for ${hadoop_process_name} to start ..."
...
expose_alluxio_client_jar() {
  ...
  # expose Alluxio client jar to Spark
...
configure_alluxio_general_properties() {
  ...
  # configure Alluxio Web UI port and RPC port, so they dont conflict with EMRs use of ports 19999 and 19998
  set_alluxio_property alluxio.master.web.port "19995"
  set_alluxio_property alluxio.master.rpc.port "19994"
}
...
EOF
  ...
  #
  # NOTE: Removed configuration of Alluxio catalog service for Presto, do not use it.
  #
